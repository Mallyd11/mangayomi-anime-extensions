...
class DefaultExtension extends MProvider {
  ...
  absolutizePlaylist(text, base) {
    return text.split("\n").map((line) => {
      const t = line.trim();
      if (!t) return line;
      // Init segments are carried on a tag rather than their own line.
      if (t.indexOf("#EXT-X-MAP:") === 0) {
        return t.replace(/URI="([^"]+)"/, (m, u) => 'URI="' + this.segmentUrl(u, base) + '"');
      }
      if (t.charAt(0) === "#") return line;
      return this.segmentUrl(t, base);
    }).join("\n");
  }
  ...
  async buildVariants(masterUrl, pk, audioPref) {
    ...
    const videoDataUri = this.toDataUri(videoPl);

    if (audios.length === 0) return [{ label: "Stream", url: videoDataUri }];
