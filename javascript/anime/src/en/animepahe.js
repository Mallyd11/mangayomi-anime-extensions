...
class DefaultExtension extends MProvider {
  ...
  async getVideoList(url) {
    ...
    try {
      ...
      // Stage 4 — kwik /e/ embed fetch. SINGLE patient attempt: kwik.cx has its
      // own interactive Cloudflare Turnstile, so the app must pop the challenge
      // and let the user solve it — firing multiple fetches (as v0.4.3 did) just
      // flashes the challenge repeatedly and the bypass fails. No custom UA: the
      // cf_clearance cookie is bound to the WebView UA (anidap's rule).
      var embed = "https://kwik.cx/e/" + ids[0];
      var kbody = "";
      try {
        var r = await this.client.get(embed, { "Referer": this.source.baseUrl + "/" });
        kbody = (r && r.body) || "";
        var st = (r && r.statusCode) || "?";
        var cfc = /Just a moment|Attention Required|challenge-platform/i.test(kbody);
        var pk = kbody.indexOf("eval(function(p,a,c,k,e,d)") >= 0;
        diag("4 kwikE: HTTP=" + st + " len=" + kbody.length + " cf=" + cfc + " packer=" + pk);
        if (!pk) return trace;
      } catch (e) {
        diag("4 kwikE: ERR " + (e && (e.message || e.name || e.toString())));
        return trace;
      }
